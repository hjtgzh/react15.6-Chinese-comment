...
var ReactCompositeComponent = {
  ...
  mountComponent: function (
    ...
    // 调用的 ReactReconcileTransaction 模块实例的 getUpdateQueue 方法
    // 返回 ReactUpdateQueue 模块
    var updateQueue = transaction.getUpdateQueue();

    // Initialize the public class
    var doConstruct = shouldConstruct(Component);
    // 初始化 Component 实例，传入 updateQueue
    ...
    if (inst.componentDidMount) {
      if (__DEV__) {
        ...
        // 调用的 ReactReconcileTransaction 模块实例的 getReactMountReady 方法，
        // 得到的是 CallbackQueue 类的实例，运行 enqueue 方法
        // 将 componentDidMount 储存到 CallbackQueue 类的实例的 _callbacks 数组内
  ...
  _constructComponentWithoutOwner: function (
    ...
    if (__DEV__) {
      ...
      // 调用 ReactBaseClasses内部ReactComponent传递updater参数
  ...
  performInitialMount: function (
    ...
    if (inst.componentWillMount) {
      if (__DEV__) {
        ...
        // 内部 setState ，只更新了组件 state， 并没有触发更新，理由如下
        inst.componentWillMount();
      }
      // When mounting, calls to `setState` by `componentWillMount` will set
      // `this._pendingStateQueue` without triggering a re-render.
      if (this._pendingStateQueue) {
        // _processPendingState 方法重置了 _pendingStateQueue 为 null
        // 后面运行 ReactUpdates 模块的 flushBatchedUpdates 方法后，
        // performUpdateIfNecessary 没有进行任何更新
    ...
    if (renderedElement === undefined) {
      // 获取 Component 内部 render 函数运行后返回的 Element对象
  ...
  updateComponent: function (
    ...
    if (willReceive && inst.componentWillReceiveProps) {
      if (__DEV__) {
        ...
        // 跟 componentWillMount 效果一样，只更新了组件 state， 并没有触发更新
        // 理由是下面执行了 _processPendingState
        inst.componentWillReceiveProps(nextProps, nextContext);
      }
    }
  ...
  _performComponentUpdate: function (
    ...
    if (hasComponentDidUpdate) {
      if (__DEV__) {
        ...
        // transaction 指的是 ReactUpdates 模块
        // flushBatchedUpdates 方法内部定义的 transaction 的 reconcileTransaction 属性
        // 将 componentDidUpdate 储存到 CallbackQueue 类的实例的 _callbacks 数组内
        transaction
          .getReactMountReady()
          .enqueue(
            inst.componentDidUpdate.bind(
