...
function batchedMountComponentIntoNode(
  ...
  // perform 第二个参数指的是，调用 mountComponentIntoNode 时绑定的this指向
  // 对应 Transaction 模块 perform 方法传入的第二个 scope 参数
  // transaction 事务在 mountComponentIntoNode 之后，
  // 调用 ReactReconcileTransaction 方法的 getTransactionWrappers，
  // 事务操作 _callbacks 数组， 运行其内部存放的 componentDidMount
  transaction.perform(
...
var ReactMount = {
  ...
  _renderNewRootComponent: function (
    ...
    // 首次加载组件的时候，此处最终会调用 ReactDefaultBatchingStrategy 模块的 batchedUpdates 方法
    // 让组件处于 batchedUpdates 执行的事务 transaction 中
    // 接下来调用 ReactUpdates 模块的 batchedUpdates 方法
    // ReactUpdates.batchedUpdates 内部调用 ReactDefaultBatchingStrategy 的 batchedUpdates 方法
    // 将队列策略 isBatchingUpdates 置为true
    // 然后在 batchedMountComponentIntoNode 方法执行后，执行 FLUSH_BATCHED_UPDATES 的close属性
    // 遍历 dirtyComponents 执行更新任务
  ...
  _renderSubtreeIntoContainer: function (
    ...
    // 获取组件实例，_renderNewRootComponent 方法将组件 isBatchingUpdates 置为true
